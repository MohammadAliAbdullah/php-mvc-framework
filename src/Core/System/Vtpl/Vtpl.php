...
defined('VTPL_PRESERVE_PHP_FORMATTING') || define('VTPL_PRESERVE_PHP_FORMATTING', true);
...
class Vtpl {
	...
	private function processTemplateFile() {
		...
		// Check if we should preserve PHP formatting
		if (defined('VTPL_PRESERVE_PHP_FORMATTING') && VTPL_PRESERVE_PHP_FORMATTING === true) {
			// Preserve formatting - be more gentle with newlines
			$this->template = preg_replace('/\n{3,}/', "\n\n", $this->template); // Reduce 3+ newlines to 2
			$this->template = preg_replace('/(?<=\=)\s*\n\s*/', ' ', $this->template); // Clean up around equals signs
		} else {
	...
	private function minifyPhp($php) {
		...
		// Check if we should preserve PHP formatting
		if (defined('VTPL_PRESERVE_PHP_FORMATTING') && VTPL_PRESERVE_PHP_FORMATTING === true) {
			// Preserve formatting - only clean up extra spaces, keep newlines
			$php = preg_replace('/[ \t]+/', ' ', $php);
			$php = preg_replace('/\n[ \t]*\n/', "\n", $php); // Reduce multiple empty lines to single
		} else {
			// Original behavior - aggressive minification
			$php = preg_replace('/\s+/', ' ', $php);
			$php = preg_replace('/\n+/', '', $php);
		}
	...
	private function minifyHtml($html) {
		...
		// Check if we should preserve PHP formatting
		if (defined('VTPL_PRESERVE_PHP_FORMATTING') && VTPL_PRESERVE_PHP_FORMATTING === true) {
			// Split the string into PHP and non-PHP parts
			$parts = preg_split('/(<\?php.*?\?>)/s', $html, -1, PREG_SPLIT_DELIM_CAPTURE);
			$result = '';
			
			foreach ($parts as $i => $part) {
				if ($i % 2 == 0) {
					// Non-PHP part - apply HTML minification
					$part = $this->minifyHtmlContent($part);
				}
				// PHP part - preserve formatting
				$result .= $part;
			}
			
			return $result;
		} else {
			// Original behavior - minify everything
			return $this->minifyHtmlContent($html);
		}
	}
	
	/**
	 * Minify HTML content without preserving PHP formatting
	 */
