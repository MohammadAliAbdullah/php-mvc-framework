...
function stripExtraSpaces($string) {
	...
	if (defined('VTPL_PRESERVE_PHP_FORMATTING') && VTPL_PRESERVE_PHP_FORMATTING === true) {
		...
		$parts = preg_split('/(<\?php.*?\?>)/s', $string, -1, PREG_SPLIT_DELIM_CAPTURE);
		$result = '';
		
		foreach ($parts as $i => $part) {
			if ($i % 2 == 0) {
				// Non-PHP part - apply space stripping
				foreach (['\t', '\n', '\r', ' '] as $space) {
					$part = preg_replace('/(' . $space . ')' . $space . '+/', '\1', $part);
				}
			}
		...
		// Original behavior - strip all extra spaces
		foreach (['\t', '\n', '\r', ' '] as $space) {
